...
- Убран двойной ring на инпутах формы: было `border-color` + `:focus-visible { outline }` одновременно, что на клике мышью и с клавиатуры давало два наложенных контура. Заменено на один `:focus` с мягким свечением через `box-shadow: 0 0 0 3px color-mix(...)` (`styles.css:761-765`). Остальные интерактивные элементы (`.close-btn`, `.method-option`, `.cta-btn`, radio-label Klarna) по-прежнему используют `:focus-visible` + `outline` — это осознанная асимметрия: у кнопок не нужен ring на клик мышью, у текстовых полей — нужен.

## Текущее состояние

- Ветка: `claude/trusting-faraday-41qnf1`, всё запушено в `origin`, HEAD = `d7aff16`.
